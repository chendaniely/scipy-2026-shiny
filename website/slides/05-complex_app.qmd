...
## Example application {footer=false}
...
Run `code/demos/app-05a-tips-layout.py`.
:::
...
## `@reactive.calc`: shared filtering
...
Sometimes you want to wait for explicit user action instead of reacting immediately:

- Filtering only after the user finishes typing
- Applying several filter changes at once

Use `ui.input_action_button()` to trigger a reactive event on click.

<https://shiny.posit.co/py/components/inputs/action-button/>

## Parts of an action button

1. UI input component: `ui.input_action_button()`
2. A function that gets called when the button is clicked
    - `@reactive.event(input.<ID>)`: ties the function to that specific button
    - `@reactive.effect`: marks it as a side effect, since a click doesn't return a value

## Exercise

See exercises: [Tips](../exercises.qmd#ex-tips) and [Tips + Button](../exercises.qmd#ex-tips-button).
